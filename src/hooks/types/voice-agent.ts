import DeviceManager from "@/rapida/hooks/types/device-manager";
...
import { Message as LocalMessage } from "@/rapida/hooks/types";
import {
...
export class VoiceAgent extends (EventEmitter as new () => TypedEmitter<AgentEventCallback>) {
  ...
  private auth: ClientAuthInfo | UserAuthInfo;
  //
  private agentMessagingId: string | undefined;

  //
  protected agent: AssistantDefinition;
  private connection:
  ...
  constructor(
    ...
    super();
    this.agent = new AssistantDefinition();
    this.agent.setAssistantid(agentId);
    this.auth = auth;
    if (agentVersion) this.agent.setVersion(agentVersion);
  ...
  async connect() {
    ...
    try {
      ...
      //
      this.connection = AssistantTalk(this.auth);
  ...
  private createAssistantRequest = (
    ...
    request.setAssistant(this.agent);
